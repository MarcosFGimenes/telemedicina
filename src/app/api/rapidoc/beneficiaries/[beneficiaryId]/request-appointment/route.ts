...
export async function GET(_request: NextRequest, ctx: { params: Promise<{ beneficiaryId: string }> }) {
  ...
  try {
    const { data } = await rapidoc.get(`/beneficiaries/${trimmed}/request-appointment`, {
    ...
    if (axios.isAxiosError(error)) {
      ...
      return NextResponse.json(
        {
          hint: 'rapidoc-request-appointment',
          upstreamStatus,
          message,
    ...
    return jsonError('rapidoc-request-appointment', 500, 'unknown error');
  }
}
